...
// 
// Related Topics Array Two Pointers 👍 6744 👎 10326
...
class N26_RemoveDuplicatesFromSortedArray {
    ...
    /*
    思路 :
    為非遞減序列, 代表一樣重複的元素會再一起
    使用兩個 pointer
    1. 目前的位置 (currentIndex)
    2. 取代的位置 (replaceIndex)
    當遇到目前位置的值與上一次位置的值"不一樣"時
    將 replaceIndex 的值取代為現在的值
    並用一個 k 紀錄 count
    ...
    class Solution {
        fun removeDuplicates(nums: IntArray): Int {
            ...
            while (currentIndex < nums.count()) {
                if (nums[currentIndex] != nums[replaceIndex - 1]) {
